...
export class ActionManager {
  ...
  public initializeChainEvents(client: BotClient): void {
    const JBR = new JuiceboxReader();
    const { chainEvents } = client.settings.paths;

    readdir(chainEvents, (err, files) => {
      if (err) Logger.error(err);
